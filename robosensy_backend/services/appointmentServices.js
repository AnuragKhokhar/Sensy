...
export default class AppointmentService {
    ...
    async addPrescription({ appointmentId, vitals, advice, testPrescribed, diagnosis, medicines, userId, markAsCompleted }) {
        try {
            ...
            const prescriptionData = await Appointment.findByIdAndUpdate(appointmentId,
                markAsCompleted === true ? {
                    ...
                    $set: { prescription: prescriptionObj },
                }, { new: true })
    ...
    /**
     * Broadcast cancellation message to multiple users
     * 
     ...
     */
    async broadcastCancellationMsg(appointmentId) {
        ...
        try {
            const appointmentData = await Appointment.find({ _id: { $in: appointmentId } }).populate('patientId doctorId hospitalId')

            for (let i = 0; i < appointmentData.length; i++) {

                const appointment = appointmentData[i]

                const { formattedDate, formattedTime } = await DateFormat.getDateTime(appointment.date)
                ...
                try {
                    const sendMsg = await SendMsg.sendWhatappMsg({
                        campaignName: 'appointment_cancel',
                        destination: appointmentData.patientId.phone,
                        userName: appointment.patientId.name,
                        templateParams
                    });
            ...
            console.log("error while broadcasting cancellation msg", error)
        }
    }
    ...
    async cancelAppointments(ids) {
        try {
            ...
            const updatedAppointments = await Appointment.aggregate([
                ...
                },
                {
                    $push: {
                        statusHistory: {
                            $each: [{
                                status: "Cancelled", date: new Date().getTime()
                            }], $position: 0
                        }
                    }
                }
            ...
            this.broadcastCancellationMsg(ids);

            return updatedAppointments;
        } catch (error) {
            console.log("error while broadcasting appointment cancelation", error);
            throw new APIError(error.name, error.httpCode, error.isOperational, error.message);
        }
    }
    ...
    async generatePrescriptionPDF(appointmentID) {
        try {
            const appointmentData = await Appointment.findOne({ _id: appointmentID }).populate('patientId doctorId hospitalId')
    ...
    async sendPrescription(appointmentID) {
        try {
            const appointmentData = await Appointment.findOne({ _id: appointmentID }).populate('patientId hospitalId doctorId')
     ...
     * Broadcast cancellation message to multiple users
     * 
     * @param {mongoose.Types.ObjectId[]} appointmentIds
     * @returns {Promise<{sendMsg: {success: boolean, submitted_message_id: string}[], totalMsg: number, totalSuccess: number, totalFailed: number, totalError: number}}>}
     */
    async broadcastPrescriptionMsg(appointmentIds) {
        ...
        try {
            const appointmentData = await Appointment.find({ _id: { $in: appointmentIds } }).populate('patientId doctorId hospitalId')

            for (let i = 0; i < appointmentData.length; i++) {

                const appointment = appointmentData[i]

                const { formattedDate } = await DateFormat.getDateTime(appointment.date)
                ...
                try {
                    ...
                    const sendMsg = await SendMsg.sendWhatsappMsg({
                        campaignName: "prescription",
                        destination: appointmentData.patientId.phone,
                        userName: appointment.patientId.name,
                        templateParams,
                        media: {
                            url: appointment.prescriptionPdfUrl,
                            filename: fileName
                        }
                    });
            ...
            console.log("error while broadcasting prescription msg", error)
        }
    }
