...
export default class patientController {
    ...
    async addPatient(req, res, next) {
        try {
            const { name, phone, dob, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference } = req.body;
            console.log('state,pincode', state, pincode);
            const newPatient = await patientServices.addNewPatient({ name, phone, dob, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference });
    ...
    async updatePatient(req, res, next) {
        try {
            ...
            const { name, phone, dob, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference } = req.body;
            const updatedPatient = await patientServices.updatePatient(patientID, { name, phone, dob, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference });
    ...
    async addUnverifiedPatient(req, res, next) {
        try {
            const { name, phone, dob, age, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference } = req.body;
            const newPatient = await patientServices.addUnverifiedPatient({ name, phone, dob, age, profilePic, emergencyContactName, emergencyContactNumber, email, govtID, address, gender, city, state, pincode, hospitalId, reference });
            return res.status(201).json({ msg: 'Patient Added Succesfully', newPatient });
        } catch (error) {
            next(error);
        }
