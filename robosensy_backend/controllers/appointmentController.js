...
export default class AppointmentController {
    ...
    addPrescription = async (req, res, next) => {
        try {
            ...
            const { markAsCompleted } = req.query

            const prescriptions = await AppoinmtentServices.addPrescription({ appointmentId, vitals, advice, testPrescribed, diagnosis, medicines, userId, markAsCompleted: markAsCompleted === 'true' })
    ...
    cancelAppointments = async (req, res, next) => {
        try {
            const { appointmentIds } = req.body
            const appointmentIdsObjectIds = appointmentIds.map(id => new mongoose.Types.ObjectId(id))

            const appointmentData = await AppoinmtentServices.cancelAppointments(appointmentIdsObjectIds)
            return res.status(201).json({ msg: 'All appointments Cancelled Successfully', appointmentData });
        } catch (error) {
            next(error)
        }
    }
    ...
    broadcastPrescription = async (req, res, next) => {
        try {
            const { appointmentIds } = req.body
            const appointmentIdsObjectIds = appointmentIds.map(id => new mongoose.Types.ObjectId(id))
            const deliveryReport = await AppoinmtentServices.broadcastPrescriptionMsg(appointmentIdsObjectIds)
            return res.status(200).json({ msg: 'Prescription broadcasted Successfully', data: deliveryReport });
        } catch (error) {
            next(error)
        }
    }
